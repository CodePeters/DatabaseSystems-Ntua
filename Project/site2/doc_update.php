    	    ...
    	    <div class="col-md-4 col-md-offset-4">
    	        <div class="panel panel-default">
    	            ...
    	            <div class="panel-body">
    	                <form class="form-horizontal" role="form" action="doc_update.php" method="post" id='f'>
						...
						<div class="form-group">
    	                    <label for="inputFormula" class="col-sm-3 control-label">
								Speciality</label>
    	                    <div class="col-sm-9">
    	                        <input type="text" class="form-control" id="inputFormula" 	placeholder="Speciality" name="spec">
    	                    </div>
    	                </div>
    	                ...
    	                <div class="form-group last">
    	                    <div class="col-sm-offset-3 col-sm-6 pull-left">
    	                        <button type="submit" class="btn btn-warning btn-sm pull-left">
									Update</button>
								 <button class="btn btn-info btn-sm pull-right"><a href="view_doc.php">Manage</a></button>
    	                    </div>
    	                </div>
    	                </form>
    	            </div>
...
<?php
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		$id = mysqli_real_escape_string($conn, trim($_POST['did']));
		
		// check if 'spec', field which is NOT set as NOT NULL
		// in the database, is empty and if it is we set it as NULL.
		if ( empty($_POST['spec']) ){
			$sp = 'NULL';
		}else{
			$sp = mysqli_real_escape_string($conn, trim($_POST['spec']));
		}
		
		if ( empty($_POST['expy']) ){
			$ex = 'NULL';
		}else{
			$ex = (int)mysqli_real_escape_string($conn, trim($_POST['expy']));
		}
		if ($ex<0){
			echo '<h3>Error! The experience years provided are not valid!</h3>';
		}else{
		// make the update query
		$q = "UPDATE Doctor SET FirstName='$fn', LastName='$ln', Speciality='$sp', ExperienceYears=$ex WHERE DoctorId=$id";
