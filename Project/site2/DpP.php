...
<?php
		...
		if (isset($_GET['p']) && is_numeric($_GET['p'])){
			...
			$q = "SELECT Patient.PatientId, Patient.LastName, Patient.FirstName, Drug.Name FROM  Patient, Drug, Prescription WHERE Patient.PatientId = Prescription.PatientId AND Drug.DrugId = Prescription.DrugId ORDER BY Patient.PatientId";
			$r = @mysqli_query($conn, $q);
			$num_rows = mysqli_num_rows($r);
			echo '<h4>Φάρμακα ανα ασθενή</h4>';
		...
		// define the query
		$q = "SELECT Patient.PatientId, Patient.LastName, Patient.FirstName, Drug.Name FROM  Patient, Drug, Prescription WHERE Patient.PatientId = Prescription.PatientId AND Drug.DrugId = Prescription.DrugId ORDER BY Patient.PatientId LIMIT $start, $display";
		$r = @mysqli_query($conn, $q);
		...
		if ($pages > 1){
			...
			// if it is not the first page make a previous link:
				if ($current_page !=1 ){
					echo '<a href="DpP.php?s=' . ($start - $display) . '&p=' . $pages . '">Previous</a> ';
				}
				// make all the numbered pages:
				for ($i=1; $i<=$pages; $i++){
					if ($i != $current_page){
						echo '<a href="DpP.php?s=' . (($display *
						($i - 1))) . '&p=' . $pages . '">' . $i . '</a> ';
					}else{
						echo $i . ' ';
					}
				...
				if ($current_page != $pages){
					echo '<a href="DpP.php?s=' . ($start + $display) . '&p=' . $pages . '">Next</a>';
				}
