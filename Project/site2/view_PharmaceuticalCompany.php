...
function insert()
{
     location.href = "http://localhost:8888/site2/PharmaceuticalCompany_insert.php";
} 
</script>

</br></br>
<!-- view_PharmaceuticalCompany.php script -->
<?php
		...
		if ( (isset($_GET['id'])) && (is_numeric($_GET['id'])) ){
			$id = $_GET['id'];
			$q = "DELETE FROM PharmaceuticalCompany WHERE PharmaceuticalCompanyId=$id LIMIT 1"; 
			$r = @mysqli_query($conn, $q);
			if (mysqli_affected_rows($conn)==1){
				echo '<p>The PharmaceuticalCompany has been successfully deleted.</p>';
			}else{ 
				echo '<p>The PharmaceuticalCompany could not be deleted due to a system error.</p>';
		...
		if (isset($_GET['p']) && is_numeric($_GET['p'])){
			...
			$q = "SELECT COUNT(PharmaceuticalCompanyId) FROM PharmaceuticalCompany";  
			$r = @mysqli_query($conn, $q);
		...
		// define the query
		$q = "SELECT PharmaceuticalCompanyId,Name,PhoneNumber FROM PharmaceuticalCompany ORDER BY PharmaceuticalCompanyId ASC LIMIT $start, $display";
		$r = @mysqli_query($conn, $q);
	
		// table headers formating
		echo '</br><table align="center" cellspacing="3" cellpadding="3" width="60%">
			<tr>
				<td align="left"><h4><b><font color="#FEDE7B">Update</font></b></h4></td>
				<td align="left"><h4><b><font color="#D64842">Delete</font></b></h4></td>
				<td align="left"><h4><b>PharmaceuticalCompanyId</b></h4></td>
				<td align="left"><h4><b>Name</b></h4></td>
				<td align="left"><h4><b>PhoneNumber</b></h4></td>
			</tr>';
		...
		while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)){
			echo '<tr>
					<td align="left"><button type="submit" class="btn btn-warning btn-xs"><a href="update1.php?id=' . $row['PharmaceuticalCompanyId'] . '"><font color="white">Update</font></a></button></td>
					<td align="left"><button type="submit" class="btn btn-danger btn-xs"><a href="view_PharmaceuticalCompany.php?id=' . $row['PharmaceuticalCompanyId'] . '"><font color="white">Delete</font></a></button></td>
					<td align="left">' . $row['PharmaceuticalCompanyId'] . '</td>
		...
		if ($pages > 1){
			...
			// if it is not the first page make a previous link:
				if ($current_page !=1 ){
					echo '<a href="view_PharmaceuticalCompany.php?s=' . ($start - $display) . '&p=' . $pages . '">Previous</a> '; 
				}
				// make all the numbered pages:
				for ($i=1; $i<=$pages; $i++){
					if ($i != $current_page){
						echo '<a href="view_PharmaceuticalCompany.php?s=' . (($display * 
						($i - 1))) . '&p=' . $pages . '">' . $i . '</a> ';
					}else{
						echo $i . ' ';
					}
				...
				if ($current_page != $pages){
					echo '<a href="view_PharmaceuticalCompany.php?s=' . ($start + $display) . '&p=' . $pages . '">Next</a>'; 
				}
				echo '</p>'; // close the paragraph
		}	
?>
<!-- end of view_PharmaceuticalCompany.php script -->
