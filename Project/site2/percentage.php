    	    ...
    	    <div class="col-md-4 col-md-offset-4">
    	        <div class="panel panel-default">
    	            <div class="panel-heading">
    	              <strong>Speciality Percentage</strong>
    	            </div>
    	            <div class="panel-body">
    	                <form class="form-horizontal" role="form" action="percentage.php" method="post">
						...
						<div class="form-group">
    	                    <label for="inputName" class="col-sm-3 control-label">
								Speciality</label>
    	                    <div class="col-sm-9">
    	                        <input type="text" class="form-control" id="inputName" 	placeholder="Speciality of Interest" required="True" name="spec">
    	                    </div>
    	                </div>
...
<?php
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		$y = mysqli_real_escape_string($conn, trim($_POST['spec']));
		...
		if (isset($_GET['p']) && is_numeric($_GET['p'])){
			...
			$q = "SELECT Count(Doctor.DoctorId) * 100 / ( SELECT COUNT(Doctor.DoctorId) FROM Doctor) as Speciality_Percentage FROM Doctor WHERE Doctor.speciality = '$y'";
		...
		while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)){
			if ( $row['Speciality_Percentage']==0.0000 ){
				echo '<h4>There is no Doctor being a '.$y.' !</h4>';
			}else{
				echo '<h4>The '.ceil($row['Speciality_Percentage']).'% of Doctors is a '.$y.' !</h4>';
			}
		}// end of while loop
		echo '</table>';
		...
		if ($pages > 1){
			...
			// if it is not the first page make a previous link:
				if ($current_page !=1 ){
					echo '<a href="percentage.php?s=' . ($start - $display) . '&p=' . $pages . '">Previous</a> ';
				}
				// make all the numbered pages:
				for ($i=1; $i<=$pages; $i++){
					if ($i != $current_page){
						echo '<a href="percentage.php?s=' . (($display *
						($i - 1))) . '&p=' . $pages . '">' . $i . '</a> ';
					}else{
						echo $i . ' ';
					}
				...
				if ($current_page != $pages){
					echo '<a href="percentage.php?s=' . ($start + $display) . '&p=' . $pages . '">Next</a>';
				}
