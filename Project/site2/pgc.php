    	    ...
    	    <div class="col-md-4 col-md-offset-4">
    	        <div class="panel panel-default">
    	            ...
    	            <div class="panel-body">
    	                <form class="form-horizontal" role="form" action="pgc.php" method="post">
						...
						<div class="form-group">
    	                    <label for="inputName" class="col-sm-3 control-label">
								City</label>
    	                    <div class="col-sm-9">
    	                        <input type="text" class="form-control" id="inputName" 	placeholder="City of Interest" required="True" name="city">
    	                    </div>
    	                </div>
...
<?php
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		$y = mysqli_real_escape_string($conn, trim($_POST['city']));
		...
		if (isset($_GET['p']) && is_numeric($_GET['p'])){
			...
			$q = "SELECT * FROM  v1 WHERE v1.Town = '$y'";
		...
		$num_rows = mysqli_num_rows($r);
		if ($num_rows==0){
			echo '<h4>Δεν υπάρχουν φαρμακεία στο ' . $y . ' !</h4>';
		}else{
		...
		if ($pages > 1){
			...
			// if it is not the first page make a previous link:
				if ($current_page !=1 ){
					echo '<a href="pgc.php?s=' . ($start - $display) . '&p=' . $pages . '">Previous</a> ';
				}
				// make all the numbered pages:
				for ($i=1; $i<=$pages; $i++){
					if ($i != $current_page){
						echo '<a href="pgc.php?s=' . (($display *
						($i - 1))) . '&p=' . $pages . '">' . $i . '</a> ';
					}else{
						echo $i . ' ';
					}
				...
				if ($current_page != $pages){
					echo '<a href="pgc.php?s=' . ($start + $display) . '&p=' . $pages . '">Next</a>';
				}
