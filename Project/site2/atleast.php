...
<?php
		...
		if (isset($_GET['p']) && is_numeric($_GET['p'])){
			...
			$q = "SELECT Drug.name FROM Drug WHERE Drug.DrugId  in  (SELECT Drug.DrugId FROM Drug, Prescription WHERE Drug.DrugId= Prescription.DrugId)";
			$r = @mysqli_query($conn, $q);
			$num_rows = mysqli_num_rows($r);
			echo '<h4>Φάρμακα που έχουν συνταγογραφηθεί από τουλάχιστον έναν γιατρό</h4>';
		...
		// define the query
		$q = "SELECT Drug.name FROM Drug WHERE Drug.DrugId  in  (SELECT Drug.DrugId FROM Drug, Prescription WHERE Drug.DrugId= Prescription.DrugId) LIMIT $start, $display";
		$r = @mysqli_query($conn, $q);
		
		
		
		// table headers formating
		echo '</br><table align="center" cellspacing="3" cellpadding="3" width="20%">
			<tr>
				<td align="left"><h4><b>Drug name</b></h4></td>
			</tr>';
		...
		if ($pages > 1){
			...
			// if it is not the first page make a previous link:
				if ($current_page !=1 ){
					echo '<a href="atleast.php?s=' . ($start - $display) . '&p=' . $pages . '">Previous</a> ';
				}
				// make all the numbered pages:
				for ($i=1; $i<=$pages; $i++){
					if ($i != $current_page){
						echo '<a href="atleast.php?s=' . (($display *
						($i - 1))) . '&p=' . $pages . '">' . $i . '</a> ';
					}else{
						echo $i . ' ';
					}
				...
				if ($current_page != $pages){
					echo '<a href="atleast.php?s=' . ($start + $display) . '&p=' . $pages . '">Next</a>';
				}
