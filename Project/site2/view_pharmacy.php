...
<?php
		...
		if (isset($_GET['p']) && is_numeric($_GET['p'])){
			...
			$q = "SELECT COUNT(PharmacyId) FROM Pharmacy";
		...
		// define the query
		$q = "SELECT PharmacyId,Name,Town,StreetName,Number,PostalCode,PhoneNumber FROM Pharmacy ORDER BY PharmacyId ASC LIMIT $start, $display";
		$r = @mysqli_query($conn, $q);
		echo '<h3>Pharmacy table</h3>';
		// table headers formating
		echo '</br><table align="center" cellspacing="3" cellpadding="3" width="60%">
			<tr>
				<td align="left"><h5><b>PharmacyId</b></h5></td>
				<td align="left"><h5><b>Name</b></h5></td>
				...
				<td align="left"><h5><b>PhoneNumber</b></h5></td>
			</tr>';
		...
		if ($pages > 1){
			...
			// if it is not the first page make a previous link:
				if ($current_page !=1 ){
					echo '<a href="view_pharmacy.php?s=' . ($start - $display) . '&p=' . $pages . '">Previous</a> ';
				}
				// make all the numbered pages:
				for ($i=1; $i<=$pages; $i++){
					if ($i != $current_page){
						echo '<a href="view_pharmacy.php?s=' . (($display *
						($i - 1))) . '&p=' . $pages . '">' . $i . '</a> ';
					}else{
						echo $i . ' ';
					}
				...
				if ($current_page != $pages){
					echo '<a href="view_pharmacy.php?s=' . ($start + $display) . '&p=' . $pages . '">Next</a>';
				}
