    	    ...
    	    <div class="col-md-4 col-md-offset-4">
    	        <div class="panel panel-default">
    	            ...
    	            <div class="panel-body">
    	                <form class="form-horizontal" role="form" action="med_insert.php" method="post">
    	                
						<div class="form-group">
    	                    <label for="inputName" class="col-sm-3 control-label">
    	                        Name</label>
    	                    <div class="col-sm-9">
    	                        <input type="text" class="form-control" id="inputName" 	placeholder="Drug's Name" required="True" name="name">
    	                    </div>
    	                </div>
						...
						<div class="form-group">
    	                    <label for="inputFormula" class="col-sm-3 control-label">
								Formula</label>
    	                    <div class="col-sm-9">
    	                        <input type="text" class="form-control" id="inputFormula" 	placeholder="Formula's Name" name="formula">
    	                    </div>
    	                </div>
						...
						<div class="form-group">
    	                    <label for="inputCompanyId" class="col-sm-3 control-label">
								Company id</label>
    	                    <div class="col-sm-9">
    	                        <input type="number" class="form-control" id="inputCompanyid" 	placeholder="Pharmaceutical Company's id" required="True" name="cid">
    	                    </div>
    	                </div>
...
<?php
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		$nm = mysqli_real_escape_string($conn, trim($_POST['name']));
		$ci = (int)mysqli_real_escape_string($conn, trim($_POST['cid']));
		
		// the formula in NOT required so in case not filled in by the user
		// it should be passed as NULL so...
		if ( empty($_POST['formula']) ){
			$fm = 'NULL';
		}else{
			$fm = mysqli_real_escape_string($conn, trim($_POST['formula']));
		}
		
		// in a similar way we can also check if the foreign key exists
		// as PharmaceuticalCompanyId in the PharmaceuticalCompanyId table
		// and if it doesn't warn the user...
		$q1 = "SELECT Name FROM PharmaceuticalCompany WHERE (PharmaceuticalCompanyId=$ci)";
		$r1 = mysqli_query($conn, $q1);
		$num1 = @mysqli_num_rows($r1);

		$q = "INSERT INTO Drug values(NULL, '$nm', '$fm', $ci)";
		...
		if (mysqli_affected_rows($conn)==1){
			...
			echo '<h1>Thank you!</h1><p>A new record has been successfuly inserted to the Drug table.</br></p>';
		}else{
			if ($num1!=1){
				echo '<h1>System Error!</h1><p>The Company Id provided is not a valid one!</p>';
				mysqli_close($conn);
			}else{
