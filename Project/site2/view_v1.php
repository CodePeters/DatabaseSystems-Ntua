...
function insert()
{
     location.href = "http://localhost:8888/site2/v1_insert.php";
}
</script>
...
<?php
		...
		if ( (isset($_GET['id'])) && (is_numeric($_GET['id'])) ){
			$id = $_GET['id'];
			$q = "DELETE FROM v1 WHERE PharmacyId=$id LIMIT 1";
		...
		if (isset($_GET['p']) && is_numeric($_GET['p'])){
			...
			$q = "SELECT COUNT(PharmacyId) FROM v1";
			$r = @mysqli_query($conn, $q);
		...
		// define the query
		$q = "SELECT Town,PharmacyId,PharmacyName FROM v1 ORDER BY PharmacyId ASC LIMIT $start, $display";
		$r = @mysqli_query($conn, $q);
		...
		echo '</br><table align="center" cellspacing="3" cellpadding="3" width="60%">
			<tr>
				...
				<td align="left"><h4><b>Town</b></h4></td>
		...
		while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)){
			echo '<tr>
					<td align="left"><button type="submit" class="btn btn-warning btn-xs"><a href="update_v1.php?id=' . $row['PharmacyId'] . '"><font color="white">Update</font></a></button></td>
					<td align="left"><button type="submit" class="btn btn-danger btn-xs"><a href="view_v1.php?id=' . $row['PharmacyId'] . '"><font color="white">Delete</font></a></button></td>
		...
		if ($pages > 1){
			...
			// if it is not the first page make a previous link:
				if ($current_page !=1 ){
					echo '<a href="view_v1.php?s=' . ($start - $display) . '&p=' . $pages . '">Previous</a> ';
				}
				// make all the numbered pages:
				for ($i=1; $i<=$pages; $i++){
					if ($i != $current_page){
						echo '<a href="view_v1.php?s=' . (($display *
						($i - 1))) . '&p=' . $pages . '">' . $i . '</a> ';
					}else{
						echo $i . ' ';
					}
				...
				if ($current_page != $pages){
					echo '<a href="view_v1.php?s=' . ($start + $display) . '&p=' . $pages . '">Next</a>';
				}
