    	    ...
    	    <div class="col-md-4 col-md-offset-4">
    	        <div class="panel panel-default">
    	            ...
    	            <div class="panel-body">
    	                <form class="form-horizontal" role="form" action="doc_insert.php" method="post">
    	                
						<div class="form-group">
    	                    <label for="inputName" class="col-sm-3 control-label">
    	                        FirstName</label>
    	                    <div class="col-sm-9">
    	                        <input type="text" class="form-control" id="inputName" 	placeholder="Doctor's FirstName" required="True" name="fname">
    	                    </div>
    	                </div>
						...
						<div class="form-group">
    	                    <label for="inputName" class="col-sm-3 control-label">
    	                        LastName</label>
    	                    <div class="col-sm-9">
    	                        <input type="text" class="form-control" id="inputName" 	placeholder="Doctor's LastName" required="True" name="lname">
    	                    </div>
    	                </div>
						...
						<div class="form-group">
    	                    <label for="inputFormula" class="col-sm-3 control-label">
								Speciality</label>
    	                    <div class="col-sm-9">
    	                        <input type="text" class="form-control" id="inputFormula" 	placeholder="Doctor's Speciality" name="spec">
    	                    </div>
    	                </div>
						...
						<div class="form-group">
    	                    <label for="inputCompanyId" class="col-sm-3 control-label">
								Experience</label>
    	                    <div class="col-sm-9">
    	                        <input type="number" class="form-control" id="inputCompanyid" 	placeholder="Experience years" name="expy">
    	                    </div>
    	                </div>
...
<?php
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		$f = mysqli_real_escape_string($conn, trim($_POST['fname']));
		$l = mysqli_real_escape_string($conn, trim($_POST['lname']));
		
		// the speciality in NOT required so in case not filled in by the user
		// it should be passed as NULL so...
		if ( empty($_POST['spec']) ){
			$s = 'NULL';
		}else{
			$s = mysqli_real_escape_string($conn, trim($_POST['spec']));
		}
		
		// the experience in NOT required so in case not filled in by the user
		// it should be passed as NULL so...
		if ( empty($_POST['expy']) ){
			$e = 'NULL';
		}else{
			// but if user doesn't ingore this field we must make sure that
			// the experience is $e>0
			$e = (int)mysqli_real_escape_string($conn, trim($_POST['expy']));
		}
		
		if( $e<0 ){
			echo '<h3>The experience years provided are not valid!</h3>';
		}else{
		

		$q = "INSERT INTO Doctor values(NULL, '$f', '$l', '$s',$e)";
		...
		if (mysqli_affected_rows($conn)==1){
			...
			echo '<h1>Thank you!</h1><p>A new record has been successfuly inserted to the Doctor table.</br></p>';
		}else{
			// another problem occured - print an error message(optional)
			echo '<h1>System Error!</h1><p>Something went wrong and the insertion was not completed!</p>';
