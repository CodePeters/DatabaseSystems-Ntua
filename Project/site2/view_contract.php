...
<?php
		...
		if (isset($_GET['p']) && is_numeric($_GET['p'])){
			...
			$q = "SELECT COUNT(PharmacyId) FROM Contract";
		...
		// define the query
		$q = "SELECT PharmacyId,PharmaceuticalCompanyId,StartDate,EndDate,Text,Supervisor FROM Contract ORDER BY PharmacyId ASC LIMIT $start, $display";
		$r = @mysqli_query($conn, $q);
		echo '<h3>Contract table</h3>';
		...
		echo '</br><table align="center" cellspacing="3" cellpadding="3" width="50%">
			<tr>
				...
				<td align="left"><h5><b>PharmaceuticalCompanyId</b></h5></td>
				<td align="left"><h5><b>StartDate</b></h5></td>
				<td align="left"><h5><b>EndDate</b></h5></td>
				<td align="left"><h5><b>Text</b></h5></td>
				<td align="left"><h5><b>Supervisor</b></h5></td>
			</tr>';
		...
		if ($pages > 1){
			...
			// if it is not the first page make a previous link:
				if ($current_page !=1 ){
					echo '<a href="view_contract.php?s=' . ($start - $display) . '&p=' . $pages . '">Previous</a> ';
				}
				// make all the numbered pages:
				for ($i=1; $i<=$pages; $i++){
					if ($i != $current_page){
						echo '<a href="view_contract.php?s=' . (($display *
						($i - 1))) . '&p=' . $pages . '">' . $i . '</a> ';
					}else{
						echo $i . ' ';
					}
				...
				if ($current_page != $pages){
					echo '<a href="view_contract.php?s=' . ($start + $display) . '&p=' . $pages . '">Next</a>';
				}
				echo '</p>'; // close the paragraph
		}	
?>
<!-- end of view_contract.php script -->
