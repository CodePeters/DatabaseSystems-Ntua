...
<?php
		...
		// define the query
		$q = "SELECT * FROM v2 LIMIT $start, $display";
		$r = @mysqli_query($conn, $q);
		...
		if ($pages > 1){
			...
			// if it is not the first page make a previous link:
				if ($current_page !=1 ){
					echo '<a href="view2.php?s=' . ($start - $display) . '&p=' . $pages . '">Previous</a> ';
				}
				// make all the numbered pages:
				for ($i=1; $i<=$pages; $i++){
					if ($i != $current_page){
						echo '<a href="view2.php?s=' . (($display *
						($i - 1))) . '&p=' . $pages . '">' . $i . '</a> ';
					}else{
						echo $i . ' ';
					}
				...
				if ($current_page != $pages){
					echo '<a href="view2.php?s=' . ($start + $display) . '&p=' . $pages . '">Next</a>';
				}
