...
<?php
		...
		if (isset($_GET['p']) && is_numeric($_GET['p'])){
			...
			$q = "SELECT * FROM v2 GROUP BY v2.PharmacyId HAVING v2.Average_Price < (SELECT avg(v2.Average_Price) FROM v2)";
			$r = @mysqli_query($conn, $q);
			$num_rows = mysqli_num_rows($r);
			echo '<h4>Φαρμακεία με μέσες τιμές φαρμάκων μικρότερες από τη μέση τιμή στα άλλα φαρμακεία</h4>';
		...
		// define the query
		$q = "SELECT * FROM v2 GROUP BY v2.PharmacyId HAVING v2.Average_Price < (SELECT avg(v2.Average_Price) FROM v2) LIMIT $start, $display";
		$r = @mysqli_query($conn, $q);
		...
		echo '</br><table align="center" cellspacing="3" cellpadding="3" width="40%">
			<tr>
				...
				<td align="left"><h4><b>Average price</b></h4></td>
			</tr>';
		...
		while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)){
			echo '<tr>
					...
					<td align="left">'  . $row['Average_Price'] . '</td>
				  </tr>';
		}// end of while loop
		echo '</table>';
		...
		if ($pages > 1){
			...
			// if it is not the first page make a previous link:
				if ($current_page !=1 ){
					echo '<a href="avgprc.php?s=' . ($start - $display) . '&p=' . $pages . '">Previous</a> ';
				}
				// make all the numbered pages:
				for ($i=1; $i<=$pages; $i++){
					if ($i != $current_page){
						echo '<a href="avgprc.php?s=' . (($display *
						($i - 1))) . '&p=' . $pages . '">' . $i . '</a> ';
					}else{
						echo $i . ' ';
					}
				...
				if ($current_page != $pages){
					echo '<a href="avgprc.php?s=' . ($start + $display) . '&p=' . $pages . '">Next</a>';
				}
