    	    ...
    	    <div class="col-md-4 col-md-offset-4">
    	        <div class="panel panel-default">
    	            ...
    	            <div class="panel-body">
    	                <form class="form-horizontal" role="form" action="dexp.php" method="post">
						...
						<div class="form-group">
    	                    <label for="inputCompanyId" class="col-sm-3 control-label">
								Years</label>
    	                    <div class="col-sm-9">
    	                        <input type="number" class="form-control" id="years" 	placeholder="Years of experience" required="True" name="years">
    	                    </div>
    	                </div>
...
<?php
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		$y = (int)mysqli_real_escape_string($conn, trim($_POST['years']));
		...
		if (isset($_GET['p']) && is_numeric($_GET['p'])){
			...
			$q = "SELECT * FROM Doctor WHERE Doctor.ExperienceYears > $y ORDER BY Doctor.ExperienceYears DESC";
		...
		$num_rows = mysqli_num_rows($r);
		if ($num_rows==0){
			echo '<h4>Δεν βρέθηκαν γιατροί με περισσότερα από ' . $y . ' χρόνια εμπειρίας.</h4>';
		}else{
		...
		echo '</br><table align="center" cellspacing="3" cellpadding="3" width="50%">
			<tr>
				...
				<td align="left"><h4><b>Years of Experience</b></h4></td>
			</tr>';
		...
		if ($pages > 1){
			...
			// if it is not the first page make a previous link:
				if ($current_page !=1 ){
					echo '<a href="dexp.php?s=' . ($start - $display) . '&p=' . $pages . '">Previous</a> ';
				}
				// make all the numbered pages:
				for ($i=1; $i<=$pages; $i++){
					if ($i != $current_page){
						echo '<a href="dexp.php?s=' . (($display *
						($i - 1))) . '&p=' . $pages . '">' . $i . '</a> ';
					}else{
						echo $i . ' ';
					}
				...
				if ($current_page != $pages){
					echo '<a href="dexp.php?s=' . ($start + $display) . '&p=' . $pages . '">Next</a>';
				}
