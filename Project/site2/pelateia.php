...
<?php
		...
		if (isset($_GET['p']) && is_numeric($_GET['p'])){
			...
			$q = "SELECT Doctor.DoctorId, Doctor.LastName, Doctor.FirstName, COUNT(Patient.PatientId) as Patients FROM Doctor, Patient WHERE (Doctor.DoctorId= Patient.DoctorId) GROUP BY Doctor.DoctorId ORDER BY count(Patient.PatientId) desc";
			$r = @mysqli_query($conn, $q);
			$num_rows = mysqli_num_rows($r);
			echo '<h4>Γιατροί με την πελατεία τους σε φθίνουσα σειρά</h4>';
		...
		// define the query
		$q = "SELECT Doctor.DoctorId, Doctor.LastName, Doctor.FirstName, COUNT(Patient.PatientId) as Patients FROM Doctor, Patient WHERE (Doctor.DoctorId= Patient.DoctorId) GROUP BY Doctor.DoctorId ORDER BY count(Patient.PatientId) desc LIMIT $start, $display";
		$r = @mysqli_query($conn, $q);
		...
		if ($pages > 1){
			...
			// if it is not the first page make a previous link:
				if ($current_page !=1 ){
					echo '<a href="pelateia.php?s=' . ($start - $display) . '&p=' . $pages . '">Previous</a> ';
				}
				// make all the numbered pages:
				for ($i=1; $i<=$pages; $i++){
					if ($i != $current_page){
						echo '<a href="pelateia.php?s=' . (($display *
						($i - 1))) . '&p=' . $pages . '">' . $i . '</a> ';
					}else{
						echo $i . ' ';
					}
				...
				if ($current_page != $pages){
					echo '<a href="pelateia.php?s=' . ($start + $display) . '&p=' . $pages . '">Next</a>';
				}
